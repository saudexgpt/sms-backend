    ...
    <div class="vx-row">
      ...
      <div class="vx-col w-full lg:w-1/3 lg:mt-0 mt-base">
        <vx-card title="Sessions By Device">
          ...
          <div slot="no-body">
            <vue-apex-charts :options="analyticsData.sessionsByDeviceDonut.chartOptions" :series="sessionsData.series" class="mt-6 mb-8" type="donut" height="325" />
          </div>

          <ul class="mt-6">
            <li v-for="deviceData in sessionsData.analyticsData" :key="deviceData.device" class="flex mb-3">
              <feather-icon :icon="deviceData.icon" :svg-classes="[`h-5 w-5 stroke-current text-${deviceData.color}`]"/>
              ...
              <div class="ml-auto flex -mr-1">
                ...
                <feather-icon :icon=" deviceData.comparedResultPercentage < 0 ? 'ArrowDownIcon' : 'ArrowUpIcon'" :svg-classes="[deviceData.comparedResultPercentage < 0 ? 'text-danger' : 'text-success' ,'stroke-current h-4 w-4 mb-1 mr-1']"/>
              </div>
            </li>
          </ul>
        </vx-card>
        ...
        <vx-card title="Chat" class="overflow-hidden">
          <template slot="no-body">
            <div class="chat-card-log">
              <component ref="chatLogPS" :is="scrollbarTag" :settings="settings" :key="$vs.rtl" class="scroll-area pt-6 px-6">
                <ul ref="chatLog">
                  <li v-for="(msg, index) in chatLog" :class="{'flex-row-reverse': msg.isSent, 'mt-4': index}" :key="index" class="flex items-start">
                    <vs-avatar :class="msg.isSent ? 'ml-5' : 'mr-5'" :src="msg.senderImg" size="40px" class="m-0 flex-shrink-0"/>
                    <div :class="{'chat-sent-msg bg-primary-gradient text-white': msg.isSent, 'border border-solid d-theme-border-grey-light': !msg.isSent}" class="msg relative bg-white shadow-md py-3 px-4 mb-2 rounded-lg max-w-md">
                      <span>{{ msg.msg }}</span>
                    </div>
                  </li>
                </ul>
            ...
            <div class="flex bg-white chat-input-container p-6">
              <vs-input v-model="chatMsgInput" class="mr-3 w-full" placeholder="Type Your Message" @keyup.enter="chatMsgInput = ''" />
              <vs-button icon-pack="feather" icon="icon-send" @click="chatMsgInput = ''"/>
            </div>
          </template>
        </vx-card>
      </div>
      ...
      <div class="vx-col w-full lg:w-1/3 lg:mt-0 mt-base">
        <vx-card title="Customers">
          ...
          <div slot="no-body">
            <!-- CHART -->
            <vue-apex-charts :options="analyticsData.customersPie.chartOptions" :series="customersData.series" type="pie" height="345" class="mt-10 mb-10" />
            ...
            <ul class="mb-1">
              <li v-for="customerData in customersData.analyticsData" :key="customerData.customerType" class="flex justify-between py-3 px-6 border d-theme-border-grey-light border-solid border-r-0 border-l-0 border-b-0">
                <span class="flex items-center">
                  <span :class="`bg-${customerData.color}`" class="inline-block h-3 w-3 rounded-full mr-2"/>
